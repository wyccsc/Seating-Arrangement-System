/*
    DB/SP/001_create_stored_procedures.sql
    ...
    Notes:
    - No dynamic SQL.
    - Write procedures do not start or commit transactions.
      The backend Service layer can wrap calls in a Transaction.
    - Validation failures use THROW so the caller can rollback.
*/

/* Purpose: Query active floors. */
CREATE OR ALTER PROCEDURE dbo.sp_get_floors
AS
BEGIN
    SET NOCOUNT ON;
...
CREATE OR ALTER PROCEDURE dbo.sp_get_seats
    @floor_id INT = NULL
AS
BEGIN
    SET NOCOUNT ON;
...
/* Purpose: Query active employees by employee number or name. */
CREATE OR ALTER PROCEDURE dbo.sp_get_employees
...
CREATE OR ALTER PROCEDURE dbo.sp_assign_seat
    @seat_id INT,
    @employee_no CHAR(5),
...
CREATE OR ALTER PROCEDURE dbo.sp_clear_seat
    @seat_id INT = NULL,
...
/* Purpose: Soft-delete an employee without an active seat assignment. */
CREATE OR ALTER PROCEDURE dbo.sp_delete_employee
    @employee_no CHAR(5)
AS
BEGIN
    SET NOCOUNT ON;
...
/* Purpose: Soft-delete a seat without an active assignment. */
CREATE OR ALTER PROCEDURE dbo.sp_delete_seat
    @seat_id INT
AS
BEGIN
    SET NOCOUNT ON;
